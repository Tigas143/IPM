...
function createTargets(target_size, horizontal_gap, vertical_gap) {
  // Extract legendas into an array
  let legendasArray = [];
  for (let i = 0; i < legendas.getRowCount(); i++) {
    ...
    legendasArray.push(legend);
  }

  // Sort legendasArray based on label length (from smaller to bigger)
  legendasArray.sort((a, b) => {
    // If the first two letters are the same, sort alphabetically
    if (a.label.substring(0, 2) === b.label.substring(0, 2)) {
      return a.label.localeCompare(b.label);
    } else {
      return a.label.substring(0, 2).localeCompare(b.label.substring(0, 2));
    }
  });
  ...
  for (var c = 0; c < GRID_COLUMNS; c++) {
    for (var r = 0; r < GRID_ROWS; r++) {
      ...
      let legendIndex = r + GRID_ROWS * c;
      let legend = legendasArray[legendIndex];

      let target = new Target(target_x, target_y + 40, target_size, legend.label, legend.id);
      targets.push(target);

      // Add the first two letters of the label above the target
      let firstTwoLetters = legend.label.substring(0, 2).toUpperCase();
      textSize(12); // Set text size for the letters
      textAlign(CENTER, CENTER); // Center the text horizontally and vertically
      fill(255); // Set text fill color to white
      text(firstTwoLetters, target_x, target_y + 40 - 20); // Display the first two letters above the target
    }
  }
}
